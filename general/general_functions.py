...
def open_suitable_files(filepath):
    ...
    if 'tif' in fileExtension:
        ...
        axes_inventory = {}
        for chanel_position, axis in enumerate(image_axes):
            axes_inventory[axis] = [chanel_position, image_shape[chanel_position]]      
        ...
        with nd2.ND2File(filepath) as infoFile:
            ...
            axes_inventory = {}
            for chanel_position, axis in enumerate(image_dimensions.keys()):
                axes_inventory[axis] = [chanel_position, image_shape[chanel_position]] 
...
def registration_with_tmat_multiD(pathsInventory, tmat_int, image_multiD, img_name, y_axis, x_axis):
    #This function uses a transformation matrix to performs registration and cropping of a 3D image
    #   input tmat_int: transormation matrix in intigers, 2D-table where
    #                   [:,0] is timpeoint
    #                   [:,1] is x-displacement    
    ...
    tifffile.imwrite(registeredFilepath, yx_croped, imagej=True)
